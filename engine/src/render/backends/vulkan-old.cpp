...
#define VK_VALIDATION_CALLBACK VKAPI_ATTR VkBool32 VKAPI_CALL
...
bool create_vulkan_instance(VulkanBackend* backend) {
    ...
    u32 total_extensions_count = needed_extension_count + std::size(extra_extensions);
...
bool create_vulkan_allocator(VulkanBackend* backend) {
...
bool setup_debug_extension(VulkanBackend* backend) {
    /*
    auto func = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(backend->instance, "vkCreateDebugUtilsMessengerEXT");
    if (func != nullptr) {
        if(func(backend->instance, &debug_create_info, nullptr, &backend->debug_messenger) != VK_SUCCESS) {
            LOG_FATAL("[VULKAN]: Failed to create debug messenger");
            return false;
        }
        return true;
    } else {
        LOG_FATAL("[VULKAN]: Failed to get debug messenger creator");
        return false;
    }*/
   return true;
}

bool create_window_surface(VulkanBackend* backend) {
...
bool select_physical_device(VulkanBackend* backend) {
    TemporalAllocator temp = TemporalAllocator::create();
...
bool select_graphics_queue(VulkanBackend* backend) {
    TemporalAllocator temp = TemporalAllocator::create();
    VkPhysicalDevice physical_device = backend->physical_device;
...
VkPresentModeKHR choose_swapchain_present_mode(VulkanBackend* backend) {
    TemporalAllocator temp = TemporalAllocator::create();
...
bool create_vulkan_swapchain(VulkanBackend* backend) {
    u32 width = backend->window->width;
    u32 height = backend->window->height;
...
VkShaderModule create_shader_module(VulkanBackend* backend, const std::string filename, ShaderKind kind) {
...
bool load_shaders(VulkanBackend* backend) {
...
bool create_render_pass(VulkanBackend* backend) {
...
bool create_graphics_pipeline(VulkanBackend* backend) {
...
bool create_frame_buffers(VulkanBackend* backend) {
...
bool create_frame_resources(VulkanBackend* backend) {
...
bool create_frame_sync_resources(VulkanBackend* backend) {
    for (u64 i = 0; i < std::size(backend->frame_resources); i++) {
        FrameResources* resources = backend->frame_resources + i;
...
BEGIN_FRAME_RESULT VulkanBackend::begin_frame() {
   ...
   vkResetCommandBuffer(resources.command_buffer, VkCommandBufferResetFlags());
...
void VulkanBackend::render_frame() {
    ...
    VkRenderPassBeginInfo render_pass_begin_info {
        ...
        .renderArea {
            .offset { 0, 0 },
            .extent {
                .width = this->swapchain_width,
                .height = this->swapchain_height,
            }
        },
        .clearValueCount = 1,
    ...
    VkViewport viewport {
        .x = 0.0f,
        .y = 0.0f,
        .width = (float)this->swapchain_width,
        .height = (float)this->swapchain_height,
        .minDepth = 0.0f,
        .maxDepth = 1.0f
    };
